-- Workforce360 AI - Base de datos Fase 4 (incidencias y solicitudes laborales)
...
CREATE TABLE marcaciones (
    ...
    id_colaborador INT UNSIGNED NOT NULL,
    id_horario INT UNSIGNED NULL COMMENT 'Snapshot del horario aplicado a la jornada',
    fecha DATE NOT NULL,
    hora_entrada DATETIME NULL,
    hora_salida DATETIME NULL,
    entrada_programada DATETIME NULL,
    salida_programada DATETIME NULL,
    tolerancia_aplicada INT UNSIGNED NOT NULL DEFAULT 0,
    horas_descanso_programadas DECIMAL(4,2) NOT NULL DEFAULT 0.00,
    ...
    resultado_entrada ENUM('PENDIENTE','PUNTUAL','TARDANZA') NOT NULL DEFAULT 'PENDIENTE',
    resultado_salida ENUM('PENDIENTE','A_TIEMPO','ANTICIPADA') NOT NULL DEFAULT 'PENDIENTE',
    ...
    INDEX idx_marcacion_horario (id_horario),
    CONSTRAINT fk_marcacion_colaborador FOREIGN KEY (id_colaborador) REFERENCES colaboradores(id_colaborador),
    CONSTRAINT fk_marcacion_horario FOREIGN KEY (id_horario) REFERENCES horarios(id_horario)
) ENGINE=InnoDB;
...
CREATE TABLE incidencias (
    ...
    CONSTRAINT fk_incidencia_revisor FOREIGN KEY (revisado_por) REFERENCES usuarios(id_usuario)
) ENGINE=InnoDB;
...
CREATE TABLE vacaciones (
    ...
    registrado_por INT UNSIGNED NULL,
    creado_en TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY uq_vacacion_incidencia (id_incidencia),
    CONSTRAINT fk_vacacion_incidencia FOREIGN KEY (id_incidencia) REFERENCES incidencias(id_incidencia),
...
CREATE TABLE licencias (
    ...
    registrado_por INT UNSIGNED NULL,
    creado_en TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY uq_licencia_incidencia (id_incidencia),
    CONSTRAINT fk_licencia_incidencia FOREIGN KEY (id_incidencia) REFERENCES incidencias(id_incidencia),
...
CREATE TABLE descansos_medicos (
    ...
    registrado_por INT UNSIGNED NULL,
    creado_en TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY uq_descanso_incidencia (id_incidencia),
    CONSTRAINT fk_descanso_incidencia FOREIGN KEY (id_incidencia) REFERENCES incidencias(id_incidencia),
...
CREATE TABLE permisos (
    ...
    registrado_por INT UNSIGNED NULL,
    creado_en TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY uq_permiso_incidencia (id_incidencia),
    CONSTRAINT fk_permiso_incidencia FOREIGN KEY (id_incidencia) REFERENCES incidencias(id_incidencia),
...
SELECT r.id_rol, p.id_permiso
FROM roles r
INNER JOIN permisos_sistema p ON p.codigo IN ('dashboard.ver','areas.gestionar','colaboradores.gestionar','horarios.gestionar','incidencias.gestionar')
WHERE r.nombre = 'RRHH';

-- Permisos operativos incorporados en Fase 3.
INSERT INTO rol_permiso (id_rol, id_permiso)
SELECT r.id_rol, p.id_permiso
FROM roles r
...
SELECT r.id_rol, p.id_permiso
FROM roles r
INNER JOIN permisos_sistema p ON p.codigo IN ('dashboard.ver','marcaciones.registrar','incidencias.gestionar')
...
-- Colaborador y acceso de demostración para probar la marcación personal.
...
-- Hash generado con password_hash('Colab123*', PASSWORD_DEFAULT).
...
SELECT 'Workforce360 AI - Fase 4 instalada correctamente' AS resultado;
