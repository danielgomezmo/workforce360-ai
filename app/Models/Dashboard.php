...
class Dashboard
{
    public static function operationalMetrics(string $date, ?int $supervisorId = null): array
    {
        ...
        foreach ($scheduled as $row) {
            ...
            $grossMinutes = max(
                0,
                intdiv(
                    $window['fin']->getTimestamp() - $window['inicio']->getTimestamp(),
                    60
                )
            );
            ...
            $breakMinutes = max(
                0,
                (int) round(((float) ($row['horas_descanso'] ?? 0)) * 60)
            );
        ...
        foreach ($marksById as $mark) {
            ...
            $earlyMinutes +=
                (int) ($mark['minutos_salida_anticipada'] ?? 0);
        ...
        $unvalidatedAbsences = max(
            0,
            $absent - $justifiedAbsences
        );

        $attendanceRate = self::percent(
            $present,
            $programmed
        );

        $punctualityRate = self::percent(
            $punctual,
            $present
        );

        $latenessRate = self::percent(
            $late,
            $programmed
        );

        $journeyCompliance = self::percent(
            $workedMinutes,
            $scheduledMinutes
        );
        ...
        foreach ($scheduledById as $id => $schedule) {
            ...
            if (isset($marksById[$id])) {
                ...
                $absenceMinutes +=
                    (int) ($marksById[$id]['minutos_tardanza'] ?? 0);

                $absenceMinutes +=
                    (int) ($marksById[$id]['minutos_salida_anticipada'] ?? 0);
            ...
            if (
                $date < date('Y-m-d') ||
                isset($absenceByCollaborator[$id])
            ) {

                $window = AttendanceRuleEngine::scheduleWindow(
                    $schedule,
                    $date
                );
                ...
                $grossMinutes = max(
                    0,
                    intdiv(
                        $window['fin']->getTimestamp()
                        - $window['inicio']->getTimestamp(),
                        60
                    )
                );
                ...
                $breakMinutes = max(
                    0,
                    (int) round(
                        ((float) ($schedule['horas_descanso'] ?? 0)) * 60
                    )
                );
        ...
        return [
            ...
            'vacaciones' =>
                (int) ($absenceCounts['VACACIONES'] ?? 0),

            'descansos_medicos' =>
                (int) ($absenceCounts['DESCANSO_MEDICO'] ?? 0),

            'licencias' =>
                (int) (
                    ($absenceCounts['LICENCIA_CON_GOCE'] ?? 0)
                    +
                    ($absenceCounts['LICENCIA_SIN_GOCE'] ?? 0)
                ),

            'permisos' =>
                (int) ($absenceCounts['PERMISO'] ?? 0),
    ...
    private static function scheduledForDate(
        string $date,
        ?int $supervisorId
    ): array {
        ...
        $sql = "
            ...
            INNER JOIN asignacion_horarios ah
                ON ah.id_colaborador = c.id_colaborador

            INNER JOIN horarios h
                ON h.id_horario = ah.id_horario

            WHERE c.fecha_ingreso <= :fecha_ingreso

              AND (
                    c.fecha_cese IS NULL
                    OR c.fecha_cese >= :fecha_cese
              )
              ...
              AND (
                    ...
                    OR ah.fecha_fin >= :fecha_asignacion_fin
              )
              ...
              AND ah.id_asignacion = (
                    ...
                    WHERE ah2.id_colaborador = c.id_colaborador
                      ...
                      AND (
                            ah2.fecha_fin IS NULL
                            OR ah2.fecha_fin >= :fecha_sub_fin
                      )

                    ORDER BY
                        ah2.es_temporal DESC,
                        ah2.fecha_inicio DESC,
                        ah2.id_asignacion DESC
        ...
        if ($supervisorId !== null) {
            ...
            $sql .= "
                AND c.id_supervisor = :supervisor
            ";
    ...
    private static function marksForDate(
        string $date,
        ?int $supervisorId
    ): array {
        ...
        $sql = "
            ...
            INNER JOIN colaboradores c
                ON c.id_colaborador = m.id_colaborador
        ...
        if ($supervisorId !== null) {
            ...
            $sql .= "
                AND c.id_supervisor = :supervisor_marcacion
            ";
    ...
    private static function approvedAbsencesForDate(
        string $date,
        ?int $supervisorId
    ): array {
    ...
    private static function workedMinutes(
        array $mark,
        string $date
    ): int {
